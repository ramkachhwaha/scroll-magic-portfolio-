...
//#region node_modules/@ashthornton/asscroll/build/asscroll.js
var require_asscroll = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function webpackUniversalModuleDefinition(root, factory) {
		if (typeof exports === "object" && typeof module === "object") module.exports = factory();
		else if (typeof define === "function" && define.amd) define([], factory);
		else if (typeof exports === "object") exports["ASScroll"] = factory();
		else root["ASScroll"] = factory();
	})(self, function() {
		return (function() {
			var __webpack_modules__ = {
				672: (function(module$1) {
					module$1.exports = {
				...
				336: (function(module$2) {
					module$2.exports = function debounce(fn, delay) {
						let timeoutID = null;
						return function() {
			...
			function __webpack_require__(moduleId) {
				...
				var module$3 = __webpack_module_cache__[moduleId] = { exports: {} };
				__webpack_modules__[moduleId](module$3, module$3.exports, __webpack_require__);
				return module$3.exports;
			}
			(function() {
				__webpack_require__.n = function(module$4) {
					var getter = module$4 && module$4.__esModule ? function() {
						return module$4["default"];
					} : function() {
						return module$4;
					};
			...
			(function() {
				...
				SelectorSet.prototype.indexes.push({
					...
					element: function getElementClassNames(el) {
						...
						if (className) {
							...
							else if (typeof className === "object" && "baseVal" in className) return className.baseVal.split(/\s/);
						}
					}
				});
				var tagRe = /^((?:[\w\u00c0-\uFFFF\-]|\\.)+)/g;
				...
				var Map;
				if (typeof window.Map === "function") Map = window.Map;
				else Map = (function() {
					function Map() {
						this.map = {};
					}
				...
				SelectorSet.prototype.matches = function(el) {
					...
					for (i = 0, len = indexes.length; i < len; i++) {
						...
						if (keys) {
							for (j = 0, len2 = keys.length; j < len2; j++) if (objs = index.map.get(keys[j])) for (k = 0, len3 = objs.length; k < len3; k++) {
								obj = objs[k];
								id = obj.id;
				...
				*/
				function makeBusStack(event) {
					if (listeners[event] === void 0) listeners[event] = [];
				}
				/**
				* Trigger a bus stack.
				*
				...
				*/
				function triggerBus(event, args) {
					if (listeners[event]) for (let i = 0; i < listeners[event].length; i++) listeners[event][i](...args);
				}
				/**
				...
				*/
				function handleDelegation(e) {
					...
					if (matches.length) for (let i = 0; i < matches.length; i++) for (let i2 = 0; i2 < matches[i].stack.length; i2++) if (nonBubblers.indexOf(e.type) !== -1) {
						addDelegateTarget(e, matches[i].delegatedTarget);
						if (e.target === matches[i].delegatedTarget) matches[i].stack[i2].data(e);
					} else {
						addDelegateTarget(e, matches[i].delegatedTarget);
				...
				*/
				class E {
					...
					*/
					bindAll(context, methods) {
						if (!methods) methods = Object.getOwnPropertyNames(Object.getPrototypeOf(context));
						for (let i = 0; i < methods.length; i++) context[methods[i]] = context[methods[i]].bind(context);
					}
					/**
					...
					*/
					on(event, el, callback, options) {
						...
						for (let i = 0; i < events.length; i++) {
							...
							for (let n = 0; n < el.length; n++) el[n].addEventListener(events[i], callback, options);
						}
					}
					/**
					...
					delegate(event, delegate, callback) {
						...
						for (let i = 0; i < events.length; i++) {
							...
							if (map === void 0) {
								...
								if (nonBubblers.indexOf(events[i]) !== -1) document.addEventListener(events[i], handleDelegation, true);
								else document.addEventListener(events[i], handleDelegation);
							}
					...
					off(event, el, callback, options) {
						...
						for (let i = 0; i < events.length; i++) {
							...
							if (typeof el === "function") {
								makeBusStack(events[i]);
								for (let n = 0; n < listeners[events[i]].length; n++) if (listeners[events[i]][n] === el) listeners[events[i]].splice(n, 1);
								continue;
							}
							...
							if (map !== void 0) {
								...
								if (map.size === 0) {
									...
									if (nonBubblers.indexOf(events[i]) !== -1) document.removeEventListener(events[i], handleDelegation, true);
									else document.removeEventListener(events[i], handleDelegation);
									continue;
								}
							}
							...
							el = maybeRunQuerySelector(el);
							for (let n = 0; n < el.length; n++) el[n].removeEventListener(events[i], callback, options);
						}
					}
					/**
				...
				class Events {
					...
					addEvents() {
						...
						if (!this.options.disableResize) src_e.on("resize", window, debounce_default()(() => {
							this.onResize();
						}, 150));
						this.onScroll();
					...
					onScroll() {
						src_e.on("wheel", window, (e) => {
							src_e.emit(Events.WHEEL, { event: e });
						}, { passive: false });
						src_e.on("scroll", window, (e) => {
							src_e.emit(Events.INTERNALSCROLL, { event: e });
						}, { passive: true });
					}
				...
				class Scrollbar {
					...
					transform() {
						let y;
						if (this.mouseDown) y = this.position;
						else {
					...
					addStyles() {
						...
						let styles = "";
						if (this.controller.options.disableNativeScrollbar) styles += `html{scrollbar-width:none;}body{-ms-overflow-style:none;}body::-webkit-scrollbar{width:0;height:0;}`;
						if (this.controller.options.scrollbarStyles) styles += `${this.controller.options.scrollbarEl} {position:fixed;top:0;right:0;width:20px;height:100%;z-index:900;}.is-touch ${this.controller.options.scrollbarEl} {display:none;}${this.controller.options.scrollbarEl} > div {padding:6px 0;width:10px;height:0;margin:0 auto;visibility:hidden;}${this.controller.options.scrollbarEl} > div > div {width:100%;height:100%;border-radius:10px;opacity:0.3;background-color:#000;}${this.controller.options.scrollbarEl} > div > div:hover {opacity:0.9;}${this.controller.options.scrollbarEl}:hover > div, ${this.controller.options.scrollbarEl}.show > div, ${this.controller.options.scrollbarEl}.active > div {visibility:visible;}${this.controller.options.scrollbarEl}.active > div > div {opacity:0.9;}`;
				...
				class Controller {
					constructor(options = {}) {
						Controller_defineProperty(this, "onScroll", ({ event }) => {
							if (!this.scrolling) {
								this.toggleIframes();
								this.scrolling = true;
							}
							...
							if (!store_default().isTouch && event.type === "wheel") {
								...
								if (store_default().isTouch && this.options.touchScrollType === "scrollTop") this.targetPos = this.horizontalScroll ? -this.containerElement.scrollLeft : -this.containerElement.scrollTop;
								else this.targetPos = -window.scrollY;
								if (store_default().isTouch && this.options.touchScrollType !== "transform") this.currentPos = this.targetPos;
							}
							this.clamp();
							if (prevTargetPos !== this.targetPos) {
								src_e.emit(Events.EXTERNALSCROLL, -this.targetPos);
								if (this.options.customScrollbar) this.scrollbar.show();
							}
						...
						Controller_defineProperty(this, "onRAF", () => {
							if (this.testFps && this.options.limitLerpRate) {
								this.time = performance.now() * .001;
								this.delta = Math.min((this.time - this.startTime) * 60, 1);
								this.startTime = this.time;
							}
							if (!this.render) return;
							if (Math.abs(this.targetPos - this.currentPos) < .5) {
							...
							} else this.currentPos += (this.targetPos - this.currentPos) * this.ease * this.delta;
						...
						Controller_defineProperty(this, "onResize", () => {
							if (this.scrollElementsLength > 1) {
								...
								this.scrollLength = (this.horizontalScroll ? bcr.right : bcr.bottom) + marginOffset - this.currentPos;
							...
							this.clamp();
							if (!this.firstResize) this.preventResizeScroll = true;
						...
						if (store_default().isTouch) {
							...
							if (this.options.touchScrollType === "transform") this.setupSmoothScroll();
					...
					setElements() {
						...
						if (this.containerElement === null) console.error("ASScroll: could not find container element");
					...
					setupSmoothScroll() {
						...
						if (this.options.customScrollbar) this.scrollbar = new Scrollbar(this);
						src_e.on(Events.INTERNALRAF, this.onRAF);
						src_e.on(Events.RESIZE, this.onResize);
						if (this.options.limitLerpRate) setTimeout(() => {
							this.testFps = false;
							this.delta = Math.round(this.delta * 10) * .1;
						}, 2e3);
					}
					applyTransform(x, y) {
						for (let i = 0; i < this.scrollElementsLength; i++) this.scrollElements[i].style.transform = `translate3d(${x}px, ${y}px, 0px)`;
					}
					enable({ newScrollElements = false, reset = false, restore = false, horizontalScroll = false } = {}) {
						...
						if (store_default().isTouch && this.options.touchScrollType !== "transform") {
							if (this.options.touchScrollType === "scrollTop") this.containerElement.style.removeProperty("overflow");
						...
						if (restore) this.scrollTo(this.prevScrollPos, false);
					...
					disable({ inputOnly = false } = {}) {
						if (!this.enabled) return;
						this.enabled = false;
						if (!inputOnly) this.render = false;
						src_e.off(Events.WHEEL, this.onScroll);
						src_e.off(Events.INTERNALSCROLL, this.onScroll);
						this.prevScrollPos = this.targetPos;
						if (store_default().isTouch && this.options.touchScrollType === "scrollTop") this.containerElement.style.overflow = "hidden";
						else store_default().body.style.height = "0px";
					}
					clamp() {
					...
					scrollTo(y, emitEvent = true) {
						this.targetPos = y;
						if (store_default().isTouch && this.options.touchScrollType !== "transform") if (this.options.touchScrollType === "scrollTop") if (this.horizontalScroll) this.containerElement.scrollTo(-this.targetPos, 0);
						else this.containerElement.scrollTo(0, -this.targetPos);
						else window.scrollTo(0, -this.targetPos);
						this.clamp();
						this.syncScroll = true;
						if (emitEvent) src_e.emit(Events.EXTERNALSCROLL, -this.targetPos);
					}
					toggleIframes(enable) {
						for (let i = 0; i < this.iframes.length; i++) this.iframes[i].style.pointerEvents = enable ? "auto" : "none";
					}
					blockScrollEvent(e) {
						e.stopPropagation();
					}
					addEvents() {
						...
						if (!store_default().isTouch) {
							...
							src_e.on("keydown", window, (e) => {
								if (e.key === "ArrowUp" || e.key === "ArrowDown" || e.key === "PageUp" || e.key === "PageDown" || e.key === "Home" || e.key === "End" || e.key === "Tab") window.scrollTo(0, -this.targetPos);
								if (e.key === "Tab") this.toggleFixedContainer();
							});
				...
				class ASScroll {
					...
					*/
					constructor(_parameters = {}) {
						...
						const { containerElement = "[asscroll-container]", scrollElements = "[asscroll]", ease = .075, touchEase = 1, touchScrollType = "none", scrollbarEl = ".asscrollbar", scrollbarHandleEl = ".asscrollbar__handle", customScrollbar = true, scrollbarStyles = true, disableNativeScrollbar = true, disableRaf = false, disableResize = false, limitLerpRate = true, blockScrollClass = ".asscroll-block" } = _parameters;
					...
					*/
					enable(parameters) {
						if (parameters !== void 0 && typeof parameters !== "object") console.warn("ASScroll: Please pass an object with your parameters. Since 2.0");
